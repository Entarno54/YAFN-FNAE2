...
FuncRemote.OnServerInvoke = function(plr,msgType,...)
	if msgType == 0x0 then -- get opponent from spot, if the player is using the spot.
		local spot = ...
		print('GetSpotRemotes (MP)')
		local SpotBindFunc,SpotEvent = BindFunc:Invoke("GetSpotRemotes",spot)
		if not (SpotBindFunc or SpotEvent) then return end
		print('GetPlayersFromSpot (MP)')
		local BF, Dad, BF2, Dad2 = SpotBindFunc:Invoke("GetPlayersFromSpot",spot)
	...
	elseif msgType == 0x2 then -- Save Settings
		...
		clientSettings = HS:JSONEncode(cacheSettings)
		--[[
...
Plrs.PlayerRemoving:Connect(function(plr)
	...
	for SettingName,Value in pairs(clientSettings) do
		if defaultSettings[SettingName] == nil then -- Removes settings that are not defined in default settings
			clientSettings[SettingName] = nil
		end
	end
	local clientKeyBinds = clientSettings.Keybinds
...
-- Sometimes Roblox will send you a message to remove a user's save data from the game
-- And this function essenstially just does that for you
wait(10)
local function purgeData(userId: number|string)
	userId = tostring(userId)
	local suc, err = pcall(function()
		local data = game:GetService("DataStoreService"):GetDataStore("PersonalInfo", 'SettingsData')
		if data then
			data:RemoveAsync("PLR_" .. userId)
		end
	end)
	if suc then 
		print("All Data Purged For " .. userId)
	else
		warn("Couldnt Remove Data For " .. userId)
	end
end