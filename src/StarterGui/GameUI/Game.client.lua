...
local LeaveButton = require(game.ReplicatedStorage.Modules.Topbar).new()
	:setLabel("Leave")
	:setName("LeaveButton")
	:align("Right")
...
local SettButton = require(game.ReplicatedStorage.Modules.Topbar).getIcon("SettingsButton")
...
local remote = RS.MultiplayerHandler or RS:FindFirstChild("MultiplayerHandler", 99999999999)
local InfoRetriever = RS.InfoRetriever or RS:WaitForChild("InfoRetriever", 99999999999999)
...
LeaveButton.toggled:Connect(SpotAction)
...
local system = channels:WaitForChild("RBXGeneral"):: TextChannel
game.ReplicatedStorage.Messager.OnClientEvent:Connect(function(message)
	local str = string.format("<font color='#FF0000'>[System] %s</font>", message)
	system:DisplaySystemMessage(str)
end)